...
// put(), get(), containsKey(), remove(), size()---> Time Complexity O(1)
import java.util.*;
    ...
    static class HashMap<K, V> { // <K, V> Generic
        ...
        private int searchInLL(K key, int bi) {
            ...
            for (int i = 0; i < ll.size(); i++) {
                Node node = ll.get(i);
                if (node.key.equals(key)) { // Fixed: Used equals() for comparison
                    return di;
                }
                di++;
            }
        ...
        public void put(K key, V value) { // Time complexity - O(lambda)-->O(1)
            ...
            if (lambda > 1) // Added condition to trigger rehash
                rehash();
        }
        ...
        public boolean containsKey(K key) { // Time Complexity--->O(1)
            int bi = hashFunction(key);
            int di = searchInLL(key, bi);
            return di != -1; // Simply return the result of the search
        }
